...
#### Motivation：
1.  Previously, The number of mobile network Base Stations(BS) was small. In the past 10 years, The Random Matrix Theory has dealt with multi-dimensional problems(Angle of arrival(AoA)) of MIMO antennas (128 antennas or more), and there are many research results[6]. However, The Smart Grid it's a large scale and many detection nodes,  there are many research results on Smart Grid analysis using Random Matrix Theory[7].
